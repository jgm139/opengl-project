...
import es.ua.eps.proyectoopengl.util.LoggerConfig;
import es.ua.eps.proyectoopengl.util.Resource3DSReader;
import es.ua.eps.proyectoopengl.util.ShaderHelper;
import es.ua.eps.proyectoopengl.util.TextResourceReader;
import es.ua.eps.proyectoopengl.util.TextureHelper;
...
public class OpenGLRenderer implements Renderer {
	...
	Resource3DSReader obj3DS_body;
	Resource3DSReader obj3DS_head;
	...
	public void onSurfaceCreated(GL10 glUnused, EGLConfig config) {
		...
		textureBody = TextureHelper.loadTexture(context, R.drawable.body_colour);
		textureHead = TextureHelper.loadTexture(context, R.drawable.head_colour);
	...
	private void drawBody() {
		...
		// Dibujamos el objeto
		for (int i=0; i<obj3DS_body.numMeshes; i++) {
			// Asociando vértices con su attribute
			final Buffer position = obj3DS_body.dataBuffer[i].position(0);
			glVertexAttribPointer(aPositionLocation, POSITION_COMPONENT_COUNT, GL_FLOAT, false, STRIDE, obj3DS_body.dataBuffer[i]);

			// Asociamos el vector de normales
			obj3DS_body.dataBuffer[i].position(POSITION_COMPONENT_COUNT);
			glVertexAttribPointer(aNormalLocation, NORMAL_COMPONENT_COUNT, GL_FLOAT, false, STRIDE, obj3DS_body.dataBuffer[i]);

			// Asociamos el vector de UVs
			obj3DS_body.dataBuffer[i].position(POSITION_COMPONENT_COUNT+NORMAL_COMPONENT_COUNT);
			glVertexAttribPointer(aUVLocation, NORMAL_COMPONENT_COUNT, GL_FLOAT, false, STRIDE, obj3DS_body.dataBuffer[i]);
			glDrawArrays(GL_TRIANGLES, 0, obj3DS_body.numVertices[i]);
		}
	}
	...
	private void drawHead() {
		...
		rotateM(modelMatrix, 0, rY, 0f, 1f, 0f);
		rotateM(modelMatrix, 0, rX, 1f, 0f, 0f);
		rotateM(modelMatrix, 0, rYhead, 0f, 1f, 0f);
		...
		// Dibujamos el objeto
		for (int i=0; i<obj3DS_head.numMeshes; i++) {
			// Asociando vértices con su attribute
			final Buffer position = obj3DS_head.dataBuffer[i].position(0);
			glVertexAttribPointer(aPositionLocation, POSITION_COMPONENT_COUNT, GL_FLOAT, false, STRIDE, obj3DS_head.dataBuffer[i]);

			// Asociamos el vector de normales
			obj3DS_head.dataBuffer[i].position(POSITION_COMPONENT_COUNT);
			glVertexAttribPointer(aNormalLocation, NORMAL_COMPONENT_COUNT, GL_FLOAT, false, STRIDE, obj3DS_head.dataBuffer[i]);

			// Asociamos el vector de UVs
			obj3DS_head.dataBuffer[i].position(POSITION_COMPONENT_COUNT+NORMAL_COMPONENT_COUNT);
			glVertexAttribPointer(aUVLocation, NORMAL_COMPONENT_COUNT, GL_FLOAT, false, STRIDE, obj3DS_head.dataBuffer[i]);
			glDrawArrays(GL_TRIANGLES, 0, obj3DS_head.numVertices[i]);
		}
	}
	...
	public void handleSeekBar(int value) {
	    rYhead = value-90;
    }
